...
function udff_admin_ui_settingOptions_toggle_switch ($settingItem_id, $helptext) {
	...
	global $$defaultVar;
	$optionItems = explode(',',$$defaultVar); 
	$settingItem_value = $$settingItem_id;
	?>
	<form autocomplete="off" name="formName_<?php echo $settingItem_id; ?>" action="javascript:void(0);" style="height:37px;">
		<input type="hidden" name="<?php echo $settingItem_id; ?>" id="<?php echo $settingItem_id; ?>" />
		<input type="hidden" class="toggleSwitch_initValue" value="<?php echo $$defaultVar; ?>" />
		<div class="labelToggleButton"><?php echo $helptext; ?></div>
		<div style="float: right;">
			<input type="checkbox" class="toggleSwitch normal" name="toggleSwitch_<?php echo $settingItem_id ?>" <?php if(intval($settingItem_value) !== 0) echo 'checked="checked"'; ?> />
		</div>
