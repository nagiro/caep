...
jQuery(document).ready(function(){
			...
			"Insert the shortcode": function() {
				...
				if ( bValid ) {
					...
					if(is_column) {
						...
						// spec change request by Chris on 13 June 2013
						// * do not insert p tags when the content is blank
						var _first_textarea = active_accordion.find('.first_textarea').val();
						if(_first_textarea) {
							tempStr = _first_textarea.replace(/(\r\n)+$/g,'');
						}
						// for all others
						...
						for (var object_value in shortcode_attributes) {
							console.log(object_value.indexOf(" ") , object_value );
							if(shortcode_attributes[object_value] && object_value.indexOf(" ") === -1) {
						...
						if (is_block_level && no_content) {
							FSA_codeToInsert += "]";
						} else if (is_block_level && tempStr !== '') {
							FSA_codeToInsert += "]</p>";
						} else {
							FSA_codeToInsert += "]";
						}
						console.log(is_block_level , tempStr !== '');
						// if it has content, then close the shortcode with the content then the closing tag
						if(!no_content) {
							
							tempStr = tempStr.replace(/\r\n/g,"\n");
							
							if ( is_block_level && tempStr !== '' ) {
								FSA_codeToInsert += "<p>" + tempStr + '</p><p>[/'+shortcode_name+']</p>';
							} else if ( is_block_level && tempStr ) {
								FSA_codeToInsert += tempStr + '[/'+shortcode_name+']</p>';
							} else {
					...
					jQuery( '.FSA_page_accordion' ).accordion("option", "active", false);
	...
	// utils for twitter timeline shortcode
	jQuery("input.Twitter.Widget.Code").hide();
	jQuery("input.Twitter.Widget.Code").after('<textarea id="TwitterWidgetCode"></textarea>');
	jQuery("textarea#TwitterWidgetCode").blur(function() {
		scrapeIt(jQuery(this).val());
	});
});

function scrapeIt(source_text) {
	var _tid_elm = jQuery("input.username");
	var _wid_elm = jQuery("input.widget_id");
	var data_widget_id = source_text.match(/data-widget-id="([^"]*)"/);
	var href = source_text.match(/href="([^"]*)"/);
	var userid = href[1].match(/https:\/\/twitter\.com\/(.*)/);
	_tid_elm.val(userid[1]);
	_wid_elm.val(data_widget_id[1]);
}