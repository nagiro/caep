		...
		init : function(ed, url) {
			...
			ed.addCommand('FSA',
				function() {
					...
					// * empty the following fields upon init
					jQuery('.FSA_page select').find('option:first').prop('selected', true);
					
					jQuery(".FSA_page input[type='text']").each ( function () {
						var _self = jQuery(this);
						_self.val(_self.prop('defaultValue'));
					});
